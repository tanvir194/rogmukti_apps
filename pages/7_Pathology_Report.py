...
with tab1:
    ...
    # Existing Patients from Billing
    st.markdown("**আগের রোগী সিলেক্ট করুন (অটো ফিল)**")
    try:
        ...
        if not patients_df.empty:
            patient_options = ["নতুন রোগী"] + patients_df['patient_name'].unique().tolist()
    ...
    uploaded_file = st.file_uploader("রিপোর্ট ফাইল আপলোড করুন (PDF/Image)", 
    ...
    if st.button("💾 রিপোর্ট সেভ করুন", type="primary", use_container_width=True):
        if patient_name.strip() and test_name.strip():
            ...
            st.error("রোগীর নাম এবং টেস্টের নাম আবশ্যক")
...
with tab2:
    ...
    try:
        ...
        st.error(f"ডাটাবেস এরর: {str(e)}")
        st.info("টেবিল তৈরি হয়েছে। এখন একটা রিপোর্ট সেভ করে দেখুন।")
