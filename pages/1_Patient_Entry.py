...
with col_n1:
    name = st.text_input("পেশেন্টের নাম (Name of the PT) *", key="pt_name_input")
    phone = st.text_input("মোবাইল নম্বর (Phone)", key="pt_phone_input")
with col_n2:
    age = st.number_input("বয়স (Age)", min_value=0, max_value=120, value=25, key="pt_age_input")
    ...
    selected_doctor_setup = st.selectbox("ডাক্তার সিলেক্ট করুন (Refd By):", doctor_options, key="pt_doc_select")

doctor = ""
if selected_doctor_setup == "অন্যান্য":
    doctor = st.text_input("✍️ নতুন ডাক্তারের নাম ও ডিগ্রি এখানে লিখুন: *", key="pt_custom_doc")
else:
...
selected_tests = st.multiselect("তালিকা থেকে টেস্ট সিলেক্ট করুন:", available_tests, key="pt_tests_select")
...
with col_c1:
    custom_name = st.text_input("কাস্টম টেস্টের নাম:", key="pt_custom_test_name")
with col_c2:
    custom_price = st.number_input("কাস্টম টেস্টের দাম:", min_value=0, value=0, step=50, key="pt_custom_test_price")
...
with col3:
    discount_amount = st.number_input("ডিসকাউন্ট (টাকা)", min_value=0, value=0, step=10, key="pt_discount_val")
    advance_paid = st.number_input("অগ্রিম পরিশোধ (Advance Paid)", min_value=0, value=0, key="pt_advance_val")
...
st.markdown("---")
submit_button = st.button("Save Bill and Go to Print (বিল সেভ করুন)", key="submit_bill_final_btn")
...
if submit_button:
    if not name or not test_with_prices:
        ...
        # সরাসরি ডেটাবেজে ডাটা রাইট করা হচ্ছে
        try:
            c.execute("""
        ...
        # 🚀 ১০০% ওয়ার্কিং ডাইনামিক সুইচ পেজ লজিক (যা আর একই পেজে আটকে রাখবে না)
        try:
            st.switch_page("pages/3_Print_Receipt.py")
        except:
            try:
                st.switch_page("3_Print_Receipt.py")
            except:
                st.info("বিলটি সেভ হয়েছে। বাম পাশের মেনু থেকে দয়া করে 'Print Receipt' অপশনে ক্লিক করুন।")
